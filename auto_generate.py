...
def get_unprocessed_articles():
    ...
    print("正在發送最高級偽裝請求獲取 RSS...")
    try:
        # 使用 impersonate="chrome110" 完美模擬真實瀏覽器特徵，繞過防火牆
        response = requests.get(SUBSTACK_FEED_URL, impersonate="chrome110", timeout=15)
        
        if response.status_code != 200:
            print(f"抓取失敗，狀態碼: {response.status_code}")
            return []
            
        # 將成功抓取到的 XML 內容交給 feedparser
